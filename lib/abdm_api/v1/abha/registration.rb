  ...
  module Abha
    class Registration < AbdmAPI::Base
      ...
      def create_health_id_with_pre_verified(email, first_name, health_id, lastName, middleName, password, profilePhoto, txn_id)
        payload = {email: otp, firstName: first_name, healthId: health_id, lastName: last_name, middleName: middle_name, password: password, profilePhoto: profile_photo, txnId: txn_id}
